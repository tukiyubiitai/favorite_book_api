...
final rakutenRepositoryProvider = Provider.family<RakutenRepository, String>(
      (_, applicationId) => RakutenRepository(applicationId),
);

class RakutenRepository {
  RakutenRepository(this.applicationId);
  ...
  Future<List<RakutenBook>> getBooks({int page = 1, String keyWord = "本"}) async {
    ...
    return items
        .map<RakutenBook>(
            (itemMap) => RakutenBook.fromJson(itemMap['Item'] as Map<String, dynamic>))
        .toList();
  }


  //追加データ取得処理
  Future<List<RakutenBook>> loadMoreBooks() async {
    // 追加の書籍を読み込むメソッド
  ...
  //検索データ取得処理
  Future<List<RakutenBook>> searchBooks(String keyWords) async {
    // 追加の書籍を読み込むメソッド
